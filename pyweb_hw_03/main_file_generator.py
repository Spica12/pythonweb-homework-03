...
parser.add_argument('-d', '--deep', default=2)             # Максимальна глибина вкладень в папках
parser.add_argument('-f', '--folders', default=2)           # Максимальна кількість папок
parser.add_argument('-i', '--files', default=10)            # Максимальна кількість файлів
...
def generate_file(path: Path):
    path_file = f'{path}/{generate_name()}.{choice(type_files).lower()}'
    with open(path_file, 'wb') as file:
        file.write('Some text'.encode())
        logging.debug(f"Added new random file: {path_file}")


def generate_archive(path: Path):
    path_file = f'{path}/{generate_name()}'
    shutil.make_archive(path_file, f'{choice(type_archives).lower()}', path)
    logging.debug(f"Added new random archive: {path_file}")
